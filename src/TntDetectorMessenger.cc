...
// ********************************************************************
//
// $Id: TntDetectorMessenger.cc 77486 2013-11-25 10:14:16Z gcosmo $
//
/// \file optical/Tnt/src/TntDetectorMessenger.cc
/// \brief Implementation of the TntDetectorMessenger class
//
//
...
TntDetectorMessenger::TntDetectorMessenger(TntDetectorConstruction* detector)
 : fTntDetector(detector)
{
  //Setup a command directory for detector controls with guidance
  fDetectorDir = new G4UIdirectory("/Tnt/detector/");
  fDetectorDir->SetGuidance("Detector geometry control");

  fVolumesDir = new G4UIdirectory("/Tnt/detector/volumes/");
  fVolumesDir->SetGuidance("Enable/disable volumes");
 
  //Various commands for modifying detector geometry
  fDimensionsCmd =
    new G4UIcmdWith3VectorAndUnit("/Tnt/detector/dimensions",this);
  ...
  fHousingThicknessCmd = new G4UIcmdWithADoubleAndUnit
    ("/Tnt/detector/housingThickness",this);
  ...
  fNxCmd = new G4UIcmdWithAnInteger("/Tnt/detector/nx",this);
  ...
  fNyCmd = new G4UIcmdWithAnInteger("/Tnt/detector/ny",this);
  ...
  fNzCmd = new G4UIcmdWithAnInteger("/Tnt/detector/nz",this);
  ...
  fSphereCmd = new G4UIcmdWithABool("/Tnt/detector/volumes/sphere",this);
  ...
  fReflectivityCmd = new G4UIcmdWithADouble("/Tnt/detector/reflectivity",this);
  ...
  fWlsCmd = new G4UIcmdWithABool("/Tnt/detector/volumes/wls",this);
  ...
  fLxeCmd = new G4UIcmdWithABool("/Tnt/detector/volumes/lxe",this);
  ...
  fNFibersCmd = new G4UIcmdWithAnInteger("/Tnt/detector/nfibers",this);
  ...
  fDefaultsCmd = new G4UIcommand("/Tnt/detector/defaults",this);
  ...
  fMainScintYield=new G4UIcmdWithADouble("/Tnt/detector/MainScintYield",this);
  ...
  fWLSScintYield = new G4UIcmdWithADouble("/Tnt/detector/WLSScintYield",this);
...
TntDetectorMessenger::~TntDetectorMessenger()
{
...
void TntDetectorMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  if( command == fDimensionsCmd ){
    fTntDetector->SetDimensions(fDimensionsCmd->GetNew3VectorValue(newValue));
  }
  else if (command == fHousingThicknessCmd){
    fTntDetector->SetHousingThickness(fHousingThicknessCmd
                                     ->GetNewDoubleValue(newValue));
  }
  else if (command == fPmtRadiusCmd){
    fTntDetector->SetPMTRadius(fPmtRadiusCmd->GetNewDoubleValue(newValue));
  }
  else if (command == fNxCmd){
    fTntDetector->SetNX(fNxCmd->GetNewIntValue(newValue));
  }
  else if (command == fNyCmd){
    fTntDetector->SetNY(fNyCmd->GetNewIntValue(newValue));
  }
  else if (command == fNzCmd){
    fTntDetector->SetNZ(fNzCmd->GetNewIntValue(newValue));
  }
  else if (command == fDefaultsCmd){
    fTntDetector->SetDefaults();
  }
  else if (command == fSphereCmd){
    fTntDetector->SetSphereOn(fSphereCmd->GetNewBoolValue(newValue));
  }
    ...
    fTntDetector->SetWLSSlabOn(fWlsCmd->GetNewBoolValue(newValue));
  }
  else if (command == fLxeCmd){
    fTntDetector->SetMainVolumeOn(fLxeCmd->GetNewBoolValue(newValue));
  }
  else if (command == fNFibersCmd){
    fTntDetector->SetNFibers(fNFibersCmd->GetNewIntValue(newValue));
  }
  else if (command == fMainScintYield){
   fTntDetector->SetMainScintYield(fMainScintYield->GetNewDoubleValue(newValue));
  }
  else if (command == fWLSScintYield){
    fTntDetector->SetWLSScintYield(fWLSScintYield->GetNewDoubleValue(newValue));
  }
}
