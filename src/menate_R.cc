...
//------------------------------------------------------------------
//
// 24 Feb 2016 - modified to compile in geant4 version 10.0.4 - BTR
// Hard code now looking for the cross sections in the MENATEG4 directory
// Added modifications for geant4.10
//


//COMMENT BY SHUYA 160509. Neutrons produced by following reactions will be tracked when N_TRACKING==1 (because these are n').
// n+12C -> n'+12C+gamma(4.4MeV)
// n+12C -> n'+ 3a
// n+12C -> n' + p + 11B (breakup)
// n+12C -> n' + n + 11C (breakup)
...
{
  //
  // Example to get the Cross Section data from environment variable set in bashrc file.
  // 2/24/16 - BTR - commented this and I hard code the directory for the cross sections.
  //
  /*
 ...
 if(!getenv("MENATEG4XS")) 
   {
     ...
     G4Exception("Program aborted in menate::ReadCrossSectionFile()", "Error",severity,"Error!");
     ...
     //G4Exception("Program aborted in menate::ReadCrossSectionFile()");
   }
 ...
 // 2/24/16 - BTR - directory where you find the cross sections.
  ...
  if(theFile.good())
  ...
  else
    {
      ...
      G4cerr << "Check directory name for finding cross section files in menate_R.cc, method ReadCrossSectionFile()" << G4endl;
      G4ExceptionSeverity severity=FatalException;
      G4Exception("Program aborted in menate_R::ReadCrossSectionFile() method!", "Error",severity,"Error!");
      //G4Exception("Program aborted in menate_R::ReadCrossSectionFile() method!");
    }
} 
...
G4ThreeVector menate_R::GenMomDir(G4ThreeVector MomDirIn, G4double theta, G4double phi)
{
  ...
  if(Norm != 1.)
    {
      // ? This trick doesn't work after geant4.9.4.p01.
...
// returns "true" if this model can be applied to the particle-type
  ...
  if (particle == *(       G4Neutron::Neutron()       )) 
    ...
    {
      G4ExceptionSeverity severity=FatalException;
      G4Exception("Error in menate_R::IsApplicable", "Error",severity,"Model \"menate_R\" can only be used for neutron scattering!");
      //G4Exception("Model \"menate_R\" can only be used for neutron scattering!");
    }
  return false;
}
...
//////////////////////////// Comment By Shuya 160525. THIS IS TO CHANGE FOR SCINTILLATION MATERIALS (1/3) /////////////////////////////////

     G4String MaterialName = theMaterial->GetName();
     if(MaterialName == "NE213")
//Comment by Shuya: NE213's composition is CH_1.213->12 + 1.213 = 13.213
       { AMass_Material = 13.213; }
//NOTE!!! by Shuya 160420. Because I named BC505 as Tnt to avoid complicated name-changing process...
//Likewise above, BC505 = CH_1.33 = 13.33
     else if(MaterialName == "Tnt")
       //{ AMass_Material = 13.33; }
//by Shuya 160421. Change from BC505 to BC519. C:H ratio is 1.728 (CH_1.728)
       //{ AMass_Material = 13.728; }
//by Shuya 160512. Change from BC519 to BC404. C:H ratio is 1.100
       //{ AMass_Material = 13.100; }
//by Shuya 160523. Change to EJ309. C:H ratio is 1.250
       { AMass_Material = 13.250; }
     else
...
//////////////////////////// Comment By Shuya 160525. THIS IS TO CHANGE FOR SCINTILLATION MATERIALS (2/3) /////////////////////////////////
//for BC505 and BC519
     //G4double NE213dens = 0.874;
     //by Shuya 160512. For BC404
     //G4double NE213dens = 1.023;
     //by Shuya 160523. For EJ309
     G4double NE213dens = 0.959;

     G4double NumAvagadro = 0.60221367;   // Num Avagadro*conversion to barns
//Comments by Shuya 160420. I didn't change this part for BC505 because BC505 density is similar to NE213 anyway. 
//Comments by Shuya 160420. This is simply getting an inverted value of number density.
...
//by Shuya 160420
//G4cout << "TESTING!!! " << aTrack.GetTrackID() << G4endl;
//Comments by Shuya 160420. I tried to kill the track except for secondaries if this is the primary particle. 
//but I couldn't. There is a compile error. Probably because aTrack is "const"... 
//Instead I decided to kill secondary neutrons in each reaction (see below)
  //if((aTrack.GetTrackID())==0)	SetTrackStatus(fStopAndKill);
...
//by Shuya 160429
  ...
  //G4int parenttrackID = aTrack().GetParentTrackID();
...
//////////////////////////// Comment By Shuya 160525. THIS IS TO CHANGE FOR SCINTILLATION MATERIALS (3/3) /////////////////////////////////
//
 if(MaterialName == "NE213")
   { AMass_Material = 13.213; }
//NOTE!!! by Shuya 160420. Because I named BC505 as Tnt to avoid complicated name-changing process...
//Likewise above, BC505 = CH_1.33 = 13.33
       ...
       //{ AMass_Material = 13.33; }
       //by Shuya 160421. Change from BC505 to BC519
       //{ AMass_Material = 13.728; }
       //by Shuya 160512. For BC404
       //{ AMass_Material = 13.100; }
       //by Shuya 160523. Change to EJ309
       { AMass_Material = 13.250; }
...
//by Shuya 160429
    //theNTrack->SetParentID(parentID);
    //thePTrack->SetParentID(parentID);
    //theNTrack->SetParentTrackID(parenttrackID);
    //thePTrack->SetParentTrackID(parenttrackID);
    //theNTrack.SetTrackID(parenttrackID);
    //thePTrack.SetTrackID(parenttrackID);
    //G4cout << "TEST PARENTID FOR NEUTRON " << parentID << G4endl;
...
//by Shuya 160420. To cut off the track after first hit by primary neutron
    //aParticleChange.AddSecondary(theNTrack);
    //Comment by Shuya 160429. Stop the neutron only if it's the first hit (parentID = 0)
    //by Shuya 160509. Not to track any secondary neutrons.
...
//by Shuya 160429
    //theNTrack->SetParentID(parentID);
    //theC12Track->SetParentID(parentID);
...
//by Shuya 160420. To cut off the track after first hit by primary neutron
 ...
 else if(ReactionName == "N_C12_NGamma")
   {
     ...
     //by Shuya 160509. Number of secondaries depend on how you track secondary neutrons.
...
//by Shuya 160509
    ...
    if(N_Tracking == 0)
    {
	//NOTE SetNumberOfSecondaries is already dealt above in the case of this reaction
    	aParticleChange.AddSecondary(theNTrack);
    }
    //Track secondary neutrons unless production by beam neutron
    else if(N_Tracking == 1)
...
//by Shuya 160429
     //theNTrack->SetParentID(parentID);
     //thePTrack->SetParentID(parentID);
     //theB11Track->SetParentID(parentID);
...
//by Shuya 160509. Change this too if you want to cut off secondary neutrons in this event.
     //aParticleChange.SetNumberOfSecondaries(3);
     //aParticleChange.SetNumberOfSecondaries(2);
//by Shuya 160420. This is not to cut off because this neutron is a reaction product by primary neutron
    //by Shuya 160509. Not to track any secondary neutrons.
     //aParticleChange.AddSecondary(theNTrack);
...
//by Shuya 160429
     //theN1Track->SetParentID(parentID);
     //theN2Track->SetParentID(parentID);
     //theC11Track->SetParentID(parentID);
...
//by Shuya 160429
     //theNTrack->SetParentID(parentID);
     //theA1Track->SetParentID(parentID);
     //theA2Track->SetParentID(parentID);
     //theA3Track->SetParentID(parentID);
