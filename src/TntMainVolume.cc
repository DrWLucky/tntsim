...
// ********************************************************************
//
// $Id: TntMainVolume.cc 82853 2014-07-14 09:07:11Z gcosmo $
//
/// \file optical/Tnt/src/TntMainVolume.cc
/// \brief Implementation of the TntMainVolume class
//
//
#include "globals.hh"
...
TntMainVolume::TntMainVolume(G4RotationMatrix *pRot,
...
//by Shuya 160414
  ...
  fScint_log = new G4LogicalVolume(fScint_box,G4Material::GetMaterial("Tnt"),
                                   "scint_log",0,0,0);
...
//by Shuya 160509. Because changed the pmt shape from tube to box.
  fPmt_x=fConstructor->GetPMTSizeX();
  fPmt_y=fConstructor->GetPMTSizeY();
...
void TntMainVolume::PlacePMTs(G4LogicalVolume* pmt_log,
...
void TntMainVolume::VisAttributes(){
...
void TntMainVolume::SurfaceProperties(){
  G4double ephoton[] = {7.0*eV, 7.14*eV};
  const G4int num = sizeof(ephoton)/sizeof(G4double);

  //**Scintillator housing properties
//Comment by Shuya 160502. Reflectivity is approximately a fraction of reflected photons / incident photons. So if it's 1, all photons will be reflected and if 0, will be absorbed. 
  G4double reflectivity[] = {fRefl, fRefl};
  assert(sizeof(reflectivity) == sizeof(ephoton));
//by Shuya 160414
//Comment by Shuya 160502. EFFICIENCY gives a quantum efficiency (= number of photoelectrons emitted / number of incident photons).
  G4double efficiency[] = {0.0, 0.0};
//COMMENT BY SHUYA 160502. This efficiency SHOULD be 0 because the code sets only photocathode has 0 efficiency so it can collect photons (see in EventActtion, how pmtSD defines the detection of photons...)!!!
  ...
  //by Shuya 160504. I set photocathodo reflectivity 0 instead of complex calculations using Real and Imaginary R-index above.
  G4double photocath_Reflectivity[] = {0.0, 0.0};
  ...
  //by Shuya 160504. I guess this name is a mistake by the original author because it is already used for a real photocath_surf. So I changed the name.
  //new G4LogicalSkinSurface("photocath_surf",fHousing_log,
  new G4LogicalSkinSurface("housing_surf",fHousing_log,
